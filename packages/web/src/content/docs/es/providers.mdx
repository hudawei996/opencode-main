...
1. Ejecute el comando `/connect` en TUI, seleccione `OpenCode Zen` y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
2. Aquí puedes seleccionar la opción **Manually enter API Key** para configurar tu acceso.
...
:::info
Existen plugins que permiten usar los modelos Claude Pro/Max con OpenCode, pero Anthropic lo prohíbe explícitamente.

Las versiones anteriores de OpenCode incluían estos plugins, pero dejaron de integrarse a partir de la versión 1.3.0.

Otras empresas ofrecen mayor libertad de elección mediante herramientas para desarrolladores; en OpenCode puedes utilizar las siguientes suscripciones sin necesidad de configuración adicional:
...
1. Dirígete al [panel de Cloudflare](https://dash.cloudflare.com/), navega hasta **AI** > **AI Gateway** y crea una nueva puerta de enlace. Anota tu **Account ID** y **Gateway ID**.

2. Ejecute el comando `/connect` y busque **Cloudflare AI Gateway**.

   ```txt
   /connect
   ```

3. Ingrese su **Account ID** cuando se le solicite.
...
4. Ingrese su **Gateway ID** cuando se le solicite.
...
5. Ingrese su **Cloudflare API token**.
...
6. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   Alternativamente, puede configurar variables de entorno en lugar de usar `/connect`:
...
Cloudflare Workers AI le permite ejecutar modelos de IA en la red global de Cloudflare directamente a través de una API REST, sin necesidad de cuentas de proveedor separadas para los modelos admitidos.

1. Dirígete al [panel de Cloudflare](https://dash.cloudflare.com/), navega hasta **Workers AI** y selecciona **Use REST API** para obtener su **Account ID** y crear un token de API.

2. Ejecute el comando `/connect` y busque **Cloudflare Workers AI**.
...
4. Ingrese su **Cloudflare API key**.
...
El soporte de GitLab Duo en OpenCode es experimental. Las características, configuración y comportamiento pueden cambiar en futuras versiones.
:::

OpenCode se integra con la [Plataforma de Agentes de GitLab Duo](https://docs.gitlab.com/user/duo_agent_platform/), proporcionando un chat basado en agentes con tecnología de IA y capacidades nativas de llamada a herramientas.

:::note[Requisitos de licencia]
La Plataforma de Agentes de GitLab Duo requiere una suscripción **Premium** o **Ultimate** de GitLab. Está disponible en GitLab.com y en GitLab Self-Managed. Consulta los [requisitos previos de la Plataforma de Agentes de GitLab Duo](https://docs.gitlab.com/user/duo_agent_platform/#prerequisites) para obtener más información.
:::
...
2. Elija su método de autenticación:
   ...
   Seleccione **OAuth** y su navegador se abrirá para la autorización.

   #### Uso del token de acceso personal
   1. Vaya a [Configuración de usuario de GitLab > Tokens de acceso](https://gitlab.com/-/user_settings/personal_access_tokens)
...
:::note
Su administrador de GitLab debe habilitar:

1. [Activar GitLab Duo](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/) para el usuario, grupo o instancia.
2. [Activar la Plataforma de Agentes](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/) (GitLab 18.8+) o [habilitar funciones beta y experimentales](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/) (GitLab 18.7 y anteriores).
3. Para Self-Managed, [configurar su instancia](https://docs.gitlab.com/administration/gitlab_duo/configure/gitlab_self_managed/).
   :::
...
##### Modelos de flujo de trabajo DAP (GitLab Duo Agent Platform)

Los modelos de flujo de trabajo DAP proporcionan una ruta de ejecución alternativa que redirige las llamadas a herramientas a través del Servicio de Flujo de Trabajo Duo de GitLab (DWS), en lugar del chat basado en agentes estándar. Cuando se selecciona un modelo `duo-workflow-*`, OpenCode:

1. Descubre los modelos disponibles en tu espacio de nombres de GitLab.
2. Muestra un selector si hay varios modelos disponibles.
3. Almacena en caché el modelo seleccionado para acelerar los inicios posteriores.
4. Redirige las solicitudes de ejecución de herramientas a través del sistema de herramientas de OpenCode controlado por permisos.

Los modelos disponibles siguen la convención de nombres `duo-workflow-*` y se descubren dinámicamente en tu instancia de GitLab.
...
NVIDIA proporciona acceso a modelos Nemotron y muchos otros modelos abiertos a través de [build.nvidia.com](https://build.nvidia.com) de forma gratuita.

1. Dirígete a [build.nvidia.com](https://build.nvidia.com), crea una cuenta y genera una clave API.

2. Ejecute el comando `/connect` y busque **NVIDIA**.

   ```txt
   /connect
   ```

3. Ingrese su clave NVIDIA API.
...
4. Ejecute el comando `/models` para seleccionar un modelo como nemotron-3-super-120b-a12b.
...
También puede usar modelos NVIDIA localmente a través de [NVIDIA NIM](https://docs.nvidia.com/nim/) configurando una URL base personalizada.
...
Como alternativa, configure su clave API como una variable de entorno.
...
1. Dirígete al [panel de LLM Gateway](https://llmgateway.io/dashboard), haz clic en **Crear clave API** y copia la clave.

2. Ejecute el comando `/connect` y busque LLM Gateway.
...
4. Muchos modelos de LLM Gateway están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
...
2. Para proveedores personalizados, verifique la configuración opencode y:
   ...
   - Se utiliza el paquete npm correcto para el proveedor. Por ejemplo, utilice `@ai-sdk/cerebras` para Cerebras. Y para todos los demás proveedores compatibles con OpenAI, utilice `@ai-sdk/openai-compatible` (para `/v1/chat/completions`); si un modelo usa `/v1/responses`, utilice `@ai-sdk/openai`. Para configuraciones mixtas bajo un solo proveedor, puede sobrescribir esta opción por modelo a través de `provider.npm`.
