...
2. Wenn keine Erweiterung übereinstimmt, wird die Shebang-Zeile auf unterstützte Interpreter überprüft.
3. Startet den entsprechenden LSP-Server, falls er noch nicht ausgeführt wird.

---

## Shebang-Erkennung

OpenCode kann die Sprache von Skripten ohne Dateierweiterung durch Parsen der Shebang-Zeile erkennen. Dies ist nützlich für Skripte, die PEP 723 Inline-Metadaten verwenden.

Unterstützte Shebang-Muster:
...
Ein Skript, das mit `#!/usr/bin/env -S uv run --script` beginnt, wird beispielsweise als Python erkannt und verwendet den pyright LSP-Server.
