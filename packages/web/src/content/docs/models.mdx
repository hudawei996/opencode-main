...
Some providers (espeically local providers) do not automatically supply information about context and output limits to OpenCode. These can be set in the model configuration directly to show context limit information. Here, a local LM Studio provider is serving Qwen 3.6 35b with a defined context window size of 200000 tokens and an output limit of 20000 tokens:

```jsonc title="opencode.jsonc" {10-13}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
