...
`/connect` コマンドを使用してプロバイダーの API キーを追加すると、それらは `~/.local/share/opencode/auth.json` に保存されます。

---

### 設定

OpenCode 設定 の `provider` セクションを通じてプロバイダーをカスタマイズできます。
...
:::note
ここでプロバイダーが見つからない場合は、 PRを送信してください。
:::
...
2. ここで **Claude Pro/Max** オプションを選択すると、ブラウザが開き、
   そして認証を求められます。
...
:::info
OpenCode で Claude Pro/Max モデルを使用できるようにするプラグインがありますが、Anthropic はこれらを明確に禁止しています。

以前のバージョンの OpenCode にはこれらのプラグインが同梱されていましたが、1.3.0 以降は同梱されなくなりました。

他の企業は開発ツールにおける自由な選択を認めており、OpenCode では以下のサブスクリプションをセットアップ不要で使用できます:
