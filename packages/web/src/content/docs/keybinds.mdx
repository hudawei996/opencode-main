...
## Desktop normal prompt input keydowns

The OpenCode desktop app's normal prompt input also has a few built-in `keydown` behaviors. These are separate from the configurable shortcuts shown in Settings -> Shortcuts.

| Key                | Behavior                                                     |
| ------------------ | ------------------------------------------------------------ |
| `cmd+u` / `ctrl+u` | Open the file picker and attach files while the normal prompt is focused |
| `!`                | Switch to shell mode when typed at the start of an empty normal prompt |
| `escape`           | Close the `@` or `/` popover, leave shell mode, abort a running response, or blur the field on macOS desktop |
| `tab`              | Accept the active `@` or `/` suggestion                      |
| `ctrl+g`           | Close the current popover or abort a running response        |
| `up` / `down`      | Go to previous input query at the start of prompt  / Go to next input query at the end of a prompt |

The up / down prompt history navigation is enabled by default and can be toggled in the Settings -> General -> Arrow keys section.
