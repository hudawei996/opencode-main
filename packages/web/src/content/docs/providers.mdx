...
OpenCode reads config from `config.json`, `opencode.json`, or `opencode.jsonc`. These files are optional and are usually created by you when you want project-specific or persistent settings. The global config directory may not exist until you create it.
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, create one of the supported config files, such as `opencode.json`:
