...
2. 如果沒有匹配的副檔名，則檢查 shebang 行以識別支援的直譯器。
3. 如果對應的 LSP 伺服器尚未執行，則自動啟動它。

---

## Shebang 偵測

OpenCode 可以透過解析 shebang 行來識別無副檔名指令碼的語言。這對於使用 PEP 723 內嵌中繼資料的指令碼非常有用。
...
例如，以 `#!/usr/bin/env -S uv run --script` 開頭的指令碼將被識別為 Python 並使用 pyright LSP 伺服器。
